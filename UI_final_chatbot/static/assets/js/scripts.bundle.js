...
// Process Queue for managing concurrent requests
...
KTDrawer.hideAll = function(skip = null) {
    var items = document.getElementsByTagName('*');
    var drawers = [];
    
    for (var i = 0; i < items.length; i++) {
        if (items[i].getAttribute('data-kt-drawer') === 'true') {
            drawers.push(items[i]);
        }
    }
...
KTDrawer.updateAll = function() {
    ...
    if (drawers && drawers.length > 0) {
        // Use process queue to manage concurrent updates
        ProcessQueue.add(function(done) {
            for (var i = 0, len = drawers.length; i < len; i++) {
                var item = drawers[i];
                var drawer = KTDrawer.getInstance(item);
                if (drawer) {
...
KTDrawer.createInstances = function() {
    ...
    for (var i = 0; i < allElements.length; i++) {
        if (allElements[i].getAttribute('data-kt-drawer') === 'true') {
...
KTDrawer.handleShow = function() {
    ...
    KTUtil.on(document.body,  '[data-kt-drawer-show="true"][data-kt-drawer-target]', 'click', function(e) {
        var targetId = this.getAttribute('data-kt-drawer-target');
        var element = null;
        
        if (targetId && targetId.startsWith('#')) {
            element = document.getElementById(targetId.substring(1));
        }
...
var KTMenu = function(element, options) {
    ...
    var _update = function() {
        var allItems = the.element.getElementsByTagName('*');
        var items = [];
        for (var i = 0; i < allItems.length; i++) {
            var item = allItems[i];
            if (item.className && item.className.indexOf('menu-item') !== -1 && 
                item.getAttribute('data-kt-menu-trigger')) {
                items.push(item);
            }
        }
...
KTMenu.hideDropdowns = function(skip) {
    ...
    for (var i = 0; i < allElements.length; i++) {
        ...
        if (el.className && el.className.indexOf('show') !== -1 && 
            el.className.indexOf('menu-dropdown') !== -1 && 
            el.getAttribute('data-kt-menu-trigger')) {
            items.push(el);
        }
    }
...
KTMenu.initGlobalHandlers = function() {
    ...
    window.addEventListener('resize', function() {
        ...
        KTUtil.throttle(timer, function() {
            ...
            for (var i = 0; i < allElements.length; i++) {
                if (allElements[i].getAttribute('data-kt-menu') === 'true') {
                    elements.push(allElements[i]);
                }
            }
    ...
    var _getAutoHeight = function() {
        ...
        if ( dependencies !== null ) {
            ...
            if (dependencies.startsWith('#')) {
                var el = document.getElementById(dependencies.substring(1));
                if (el) elements.push(el);
            } else if (dependencies.startsWith('.')) {
                var className = dependencies.substring(1);
                for (var j = 0; j < allElements.length; j++) {
                    if (allElements[j].className && allElements[j].className.indexOf(className) !== -1) {
        ...
        if ( wrappers !== null ) {
            ...
            // Manual selector parsing for basic cases
            if (wrappers.startsWith('#')) {
                var el = document.getElementById(wrappers.substring(1));
                if (el) elements.push(el);
            } else if (wrappers.startsWith('.')) {
                var className = wrappers.substring(1);
                for (var j = 0; j < allElements.length; j++) {
                    if (allElements[j].className && allElements[j].className.indexOf(className) !== -1) {
                        elements.push(allElements[j]);
                    }
...
KTScroll.createInstances = function() {
    ...
    for (var i = 0; i < allElements.length; i++) {
        if (allElements[i].getAttribute('data-kt-scroll') === 'true') {
            elements.push(allElements[i]);
        }
    }
...
KTScrolltop.createInstances = function() {
    ...
    for (var i = 0; i < allElements.length; i++) {
        if (allElements[i].getAttribute('data-kt-scrolltop') === 'true') {
            elements.push(allElements[i]);
        }
    }
...
KTSticky.createInstances = function() {
    ...
    for (var i = 0; i < allElements.length; i++) {
        if (allElements[i].getAttribute('data-kt-sticky') === 'true') {
            elements.push(allElements[i]);
        }
    }
...
KTSwapper.createInstances = function() {
    ...
    for (var i = 0; i < allElements.length; i++) {
        if (allElements[i].getAttribute('data-kt-swapper') === 'true') {
            elements.push(allElements[i]);
        }
    }
...
KTToggle.createInstances = function() {
    ...
    for (var i = 0; i < allElements.length; i++) {
        if (allElements[i].getAttribute('data-kt-toggle')) {
            elements.push(allElements[i]);
        }
    }
...
if (!Element.prototype.matches) {
    Element.prototype.matches = function(s) {
        var allElements = (this.document || this.ownerDocument).getElementsByTagName('*');
        var matches = [];
        for (var j = 0; j < allElements.length; j++) {
            if (allElements[j].matches && allElements[j].matches(s)) {
                matches.push(allElements[j]);
            }
        }
            i = matches.length;
    ...
    return {
        ...
        findAll: function(parent, query) {
            if ( parent !== null ) {
                ...
                var results = [];
                for (var i = 0; i < allElements.length; i++) {
                    if (allElements[i].matches && allElements[i].matches(query)) {
        ...
        matches: function(el, selector, log) {
            ...
            var f = p.matches || p.webkitMatchesSelector || p.mozMatchesSelector || p.msMatchesSelector || function(s) {
                var allElements = document.getElementsByTagName('*');
                for (var i = 0; i < allElements.length; i++) {
                    if (allElements[i] === this && allElements[i].matches && allElements[i].matches(s)) {
                        return true;
                    }
                }
                return false;
        ...
        on: function(element, selector, event, handler) {
            ...
            window.KTUtilDelegatedEventHandlers[eventId] = function(e) {
                ...
                var targets = [];
                for (var i = 0; i < allElements.length; i++) {
                    if (allElements[i].matches && allElements[i].matches(selector)) {
                        targets.push(allElements[i]);
                    }
                }
                var target = e.target;
...
var KTApp = function() {
    ...
    var initBootstrapTooltips = function(el, options) {
        ...
        for (var i = 0; i < allElements.length; i++) {
            if (allElements[i].getAttribute('data-bs-toggle') === 'tooltip') {
                tooltipTriggerList.push(allElements[i]);
            }
        }
    ...
    var initBootstrapPopovers = function() {
        ...
        for (var i = 0; i < allElements.length; i++) {
            if (allElements[i].getAttribute('data-bs-toggle') === 'popover') {
                popoverTriggerList.push(allElements[i]);
            }
        }
    ...
    var initScrollSpy = function() {
        ...
        var elements = [];
        for (var i = 0; i < allElements.length; i++) {
            if (allElements[i].getAttribute('data-bs-spy') === 'scroll') {
                elements.push(allElements[i]);
            }
        }
    ...
    var initButtons = function() {
        ...
        var buttonsGroup = [];
        for (var i = 0; i < allElements.length; i++) {
            if (allElements[i].getAttribute('data-kt-buttons') === 'true') {
                buttonsGroup.push(allElements[i]);
            }
        }
        ...
        buttonsGroup.map(function (group) {
            ...
            KTUtil.on(group, selector, 'click', function(e) {
                // Find all active buttons matching the selector within the group
                var allGroupElements = group.getElementsByTagName('*');
                var activeButtons = [];
                
                for (var i = 0; i < allGroupElements.length; i++) {
                    var el = allGroupElements[i];
                    var matchesSelector = false;
                    
                    // Check if element matches the selector
                    if (selector.startsWith('.')) {
                        var className = selector.substring(1);
                        matchesSelector = el.className && el.className.indexOf(className) !== -1;
                    } else if (selector.startsWith('#')) {
                        matchesSelector = el.id === selector.substring(1);
                    } else {
                        matchesSelector = el.tagName && el.tagName.toLowerCase() === selector.toLowerCase();
                    }
                    
                    // If it matches selector AND has active class, add to list
                    if (matchesSelector && el.className && el.className.indexOf('active') !== -1) {
                        activeButtons.push(el);
                    }
                }

                // Remove active class from all found buttons
    ...
    var initCheck = function() {
        ...
        KTUtil.on(document.body,  '[data-kt-check="true"]', 'change', function(e) {
            var check = this;
            var targetSelector = check.getAttribute('data-kt-check-target');
            var allElements = document.getElementsByTagName('*');
            var targets = [];
            
            if (targetSelector.startsWith('#')) {
                var el = document.getElementById(targetSelector.substring(1));
    ...
    var initSelect2 = function() {
        ...
        var elements = [];
        for (var i = 0; i < allElements.length; i++) {
            if (allElements[i].getAttribute('data-control') === 'select2' || 
                allElements[i].getAttribute('data-kt-select2') === 'true') {
                elements.push(allElements[i]);
            }
        }
    ...
    var initAutosize = function() {
        ...
        var inputs = [];
        for (var i = 0; i < allElements.length; i++) {
            if (allElements[i].getAttribute('data-kt-autosize') === 'true') {
    ...
    var initCountUp = function() {
        ...
        var elements = [];
        for (var i = 0; i < allElements.length; i++) {
            if (allElements[i].getAttribute('data-kt-countup') === 'true' && 
                !allElements[i].className.includes('counted')) {
                elements.push(allElements[i]);
            }
        }
    ...
    var initCountUpTabs = function() {
        ...
        var tabs = [];
        for (var i = 0; i < allElements.length; i++) {
            if (allElements[i].getAttribute('data-kt-countup-tabs') === 'true' && 
                allElements[i].getAttribute('data-bs-toggle') === 'tab') {
    ...
    var initTinySliders = function() {
        ...
        var elements = [];
        for (var i = 0; i < allElements.length; i++) {
            if (allElements[i].getAttribute('data-tns') === 'true') {
                elements.push(allElements[i]);
            }
        }
