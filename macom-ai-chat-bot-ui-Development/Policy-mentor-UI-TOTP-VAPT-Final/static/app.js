...
class AuthService {
    ...
    static async auth_token_chat(auth_token) {
        try {
            debugger;
            // const encrypted = this.encryptCredentials(userName, password);
            const response = await fetch(`${CONFIG.API_BASE_URL}/validate-token-chat`, {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                    'Authorization': `Bearer ${auth_token}`, // Ensure accessToken is correctly set
...
class ChatService {
    ...
    static async getBotResponse(message) {
        try {
            ...
            window.location.href = '/Macom-policy-mentor';
...
class ChatUI {
    ...
    addMessageToChat(message, type) {
        ...
        // First approach: If you want to keep markdown but prevent HTML injection
        // 1. Sanitize the original message (escape HTML characters)
        ...
        // 2. Parse the sanitized message with marked
        const sanitizedMessage = sanitizeInput(message);
        // Set options to prevent marked from allowing raw HTML
        const markedOptions = {
            sanitize: true, // This might be deprecated in newer versions of marked
            allowDangerousHtml: false // Use this for newer versions of marked
        };
        
        // 3. Now you can safely set innerHTML with the sanitized markdown HTML
        messageElement.innerHTML = marked.parse(sanitizedMessage, markedOptions);
        
        // Alternative approach: Using DOMPurify (recommended for production)
        // Uncomment this section and comment out steps 1-3 above if using DOMPurify
        /*
        // First parse markdown
        const markedOutput = marked.parse(message);
        // Then sanitize the HTML output
        messageElement.innerHTML = DOMPurify.sanitize(markedOutput);
        */
...
async function initializeChatPage() {
    ...
    try {
        const response = await AuthService.auth_token_chat(token);
...
if (loginForm) {
    ...
    if (window.location.pathname.includes('/redirect')) {
		...
		debugger;
                window.location.href = '/Macom-policy-mentor'; // Redirect if no token
                return;
            }
            ...
            try {
                ...
                alert(error.message || 'Authentication failed');
		debugger;
                window.location.href = '/Macom-policy-mentor'; // Redirect on auth failure
            }
        }
