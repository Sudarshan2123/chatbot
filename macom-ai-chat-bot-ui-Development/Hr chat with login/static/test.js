...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (chatContainer && userInput && sendButton) {
        new ChatService(chatContainer, userInput, sendButton);
    }
...
class Chatbox {
    ...
    login() {
        debugger;
        const login_url = `${CONFIG.API_BASE_URL}/login`; 
        // const login_url='${API_BASE_URL}/login'
        this.args.loading_spinner.style.display = 'block';
        fetch(login_url, {
            method: 'POST',
            body: JSON.stringify({
                Phone_number: encodeURIComponent(this.state.Phone_number),
                otp: encodeURIComponent(this.state.otp)
            }),
            mode: 'cors',
            headers: {
                'Content-Type': 'application/json'
        ...
        .then(({ data, token }) => {
            if (data.status === "success") {
                ...
                let successMessage = { name: "Mia", message: "Hi. My name is MACOM AI. How can I help you?" };
        ...
        .catch((error) => {
            ...
            let failedMessage = { name: "Mia", message: "sorry you can not chat with MACOM AI right now !" };
            this.messages.push(failedMessage);
            const chatBox = this.args.chatBox;
            this.updateChatText(chatBox);
            console.error('Login failed');
    ...
    onRefreshButton(chatbox) {
        ...
        const clear_history_url=`${CONFIG.API_BASE_URL}/clear_history`
       ...
       startVoiceRecognition(chatbox) {
        ...
        this.recognition.onresult = (event) => {
            ...
            this.translateText(speechResult, 'en', (translatedText) => {
                ...
                const chaturl = `${CONFIG.API_BASE_URL}/chat`;
                fetch(chaturl, {
                    method: 'POST',
                    body: JSON.stringify({
                        input: encodeURIComponent(translatedText),
    ...
    translateText(text, targetLang, callback) {
        ...
        const url=`${CONFIG.API_BASE_URL}/translate`
        const requestBody = {
    ...
    textToSpeech(text) {
        ...
        const url=`${CONFIG.API_BASE_URL}/text-to-speech`
        const requestBody = {
            text: encodeURIComponent(text),
