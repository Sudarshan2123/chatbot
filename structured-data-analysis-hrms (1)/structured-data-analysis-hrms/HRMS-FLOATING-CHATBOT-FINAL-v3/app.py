...
async def chat_endpoint(request: Request, data: ChatRequest2 = Body(...)):
    ...
    try:
        ...
        # LOGGING TO VERIFY FETCH
        logging.info(f"Chat2 Endpoint - Received: user_id={user_id}, firm_id={firm_id}, role_id={role_id}")

        pipeline = ChatbotPipeline()
        # You'll need to update main_chatbot signature to accept these new params if needed
        # For now, we assume it just takes the existing ones, or you can update it later
...
async def chat_stream_endpoint(request: Request, data: ChatRequest2 = Body(...)):
    ...
    try:
        ...
        # 1. Fetch parameters from the validated data object
        user_input = urllib.parse.unquote(data.input)
        lang = urllib.parse.unquote(data.lang)
        user_id = data.user_id
        firm_id = data.firm_id
        ...
        # 2. LOGGING: This helps you verify if they are being correctly fetched
        logging.info(f"STREAM REQUEST - User: {user_id}, Firm: {firm_id}, Role: {role_id}")

        async def generate_response():
            try:
                pipeline = ChatbotPipeline()
                # Pass the new parameters to generate_content_stream
                async for chunk in pipeline.generate_content_stream(
                    sanitized_input,
                    user_id,
                    lang, firm_id, role_id):
                    yield f"data: {chunk}\n\n"
                yield "data: [DONE]\n\n"
            except Exception as err:
