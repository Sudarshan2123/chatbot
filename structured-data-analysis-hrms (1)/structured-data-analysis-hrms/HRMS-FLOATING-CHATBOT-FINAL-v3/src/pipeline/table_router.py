...
class TableRouter:
    ...
    def route_tables(
        ...
        try:
            ...
            logger.info(f"Final routing decision: {len(decision.relevant_tables)} tables selected")
            logger.info(f"Tables: {decision.relevant_tables}")
            logger.info(f"Analysis type: {decision.analysis_type.value}")
            return decision
    ...
    def _create_table_summary(
        ...
        for name, metadata in available_tables.items():
            ...
            relationship_hints = []
            for col in metadata.columns[:15]:  # Check more columns for relationships
                col_lower = col.lower()
                if col_lower.endswith('_id') or col_lower.endswith('id'):
                    relationship_hints.append(f"{col} (likely FK)")
                elif 'manager' in col_lower or 'team' in col_lower or 'supervisor' in col_lower:
                    relationship_hints.append(f"{col} (team/hierarchy)")
            ...
            if relationship_hints:
                summary += f"\n  Key Relationships: {', '.join(relationship_hints[:3])}"
    ...
    3. **ALWAYS consider multiple related tables** - especially for queries about:
       - Employee teams/managers (need employee master + team/manager tables)
       - Employee details with attendance (need employee master + attendance tables)
       - Any query requiring joins between master and detail tables
    4. Prioritize tables that have direct relationships (e.g., foreign keys).
    5. The output **MUST NOT** be wrapped in markdown code fences (e.g., ```json...```).

    **SPECIAL ROUTING PATTERNS:**
    - Team/Manager queries: Always include employee master table + any team/manager/hierarchy tables
    - Employee details: Always include employee master table + relevant detail tables
    - Attendance with employee info: Include both employee master and attendance tables
    - Department queries: Include employee master + department master tables
    ...
    **FINAL COMMAND: Output ONLY the JSON object, starting with '{{' and ending with '}}'.**"""
    ...
    def _validate_and_build_decision(self, data: Dict) -> Optional[RoutingDecision]:
        ...
        try:
            ...
            logger.info(f"Table router selected {len(tables)} tables: {tables}")
