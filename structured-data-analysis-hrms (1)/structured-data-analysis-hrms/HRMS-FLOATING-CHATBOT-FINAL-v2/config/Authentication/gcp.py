...
def load_gcp_credentials() -> Optional[service_account.Credentials]:
    ...
    for attempt in range(1, MAX_RETRIES + 1):
        ...
        except (auth_exceptions.GoogleAuthError, ValueError, OSError) as e:
            logging.error(f"Attempt {attempt}: Credential loading error: {e}")
            if attempt == MAX_RETRIES:
                return None
            time.sleep(RETRY_DELAY_SECONDS)
        except Exception as e:
            logging.critical(f"Attempt {attempt}: Critical error loading GCP credentials: {e}")
            return None
