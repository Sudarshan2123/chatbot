...
class QuerySanitizer:
    ...
    @staticmethod
    def validate_employee_access(query: str, emp_code: str) -> bool:
        """Validate that query only accesses user's own data"""
        query_lower = query.lower()

        # 1. FIX: Explicitly ALLOW if user asks for their own ID
        # This prevents accidental blocking if the query structure is complex
        if emp_code and str(emp_code) in query:
            return True

        # 2. Allow self-referential terms
        if 'my ' in query_lower or ' me ' in query_lower or 'myself' in query_lower:
            return True

        # 3. Check for attempts to access other employees/aggregate data
        forbidden_patterns = [
            'all employees', 'other employees', 'team members',
            'everyone', 'all users', 'employee list', 'all staff'
        ]
