...
class DatabaseManager:
    ...
    # In src/pipeline/database_manager.py

    def get_table_metadata_for_user(self, table_name, emp_code):
        """
        Gives the LLM a sample that ONLY contains the user's own data.
        """
        metadata = self.get_table_metadata(table_name)
        # Overwrite the sample data so the LLM only sees this user's record format
        query = f"SELECT * FROM hrms.{table_name} WHERE employee_code = :code LIMIT 1"
        # ... execute and update metadata['sample_data'] ...
        return metadata
